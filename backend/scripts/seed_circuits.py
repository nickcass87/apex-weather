"""Seed the database with circuit data from the project PDFs."""
import sys
import os
...
CIRCUITS = [
    # --- Original circuits (weather-sensitive beta, priority from launch strategy PDF) ---
...
def seed():
    ...
    try:
        ...
        if existing > 0:
            print(f"Database already has {existing} circuits. Skipping seed.")
            return
        ...
        db.commit()
        print(f"Seeded {len(CIRCUITS)} circuits.")
    finally:
        db.close()
